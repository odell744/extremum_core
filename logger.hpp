	...
	namespace core
	{
		...
		public:
				...
				m_file_path("C:\\EXFiles")
			{
				...
				{
					SetConsoleTitleA("extremum::logger");
			...
			void log(log_color color, std::string_view prefix, std::string_view format, Args const&...args)
			{
				...
				this->raw_to_console(color, console_timestamp, " [", prefix, "] ", message, "\n");
				this->raw_to_file(file_timestamp, " [", prefix, "] ", message, "\n");
			}
		private:
			template <typename ...Args>
...
#define LOG_ERROR_IMPL(format, ...) (::extremum::core::log_error(format, __VA_ARGS__))
#define LOG_ERROR(format, ...) LOG_ERROR_IMPL(format, __VA_ARGS__)

#define LOG_TRACE_IMPL(format, ...) (::extremum::core::log_trace(format, __VA_ARGS__))
#define LOG_TRACE(format, ...) LOG_TRACE_IMPL(format, __VA_ARGS__)

#define LOG_RAW_IMPL(color, ...) (::extremum::core::log_raw(color, __VA_ARGS__))
